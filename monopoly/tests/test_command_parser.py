...
def test_parse_save_command() -> None:
    parser = CommandParser()
    command, arguments = parser.parse("save")
...
def test_parse_load_command() -> None:
    parser = CommandParser()
    command, arguments = parser.parse("load")

    assert command == "load"
    assert arguments == []


def test_parse_keeps_argument_case() -> None:
    parser = CommandParser()
    command, arguments = parser.parse("SAVE MySaveFile")

    assert command == "save"
    assert arguments == ["MySaveFile"]
